...
from servers.shared.schemas import LiveFrameMessage
...
async def handle_live_frame_message(websocket: WebSocket, session_id: str, payload: dict[str, object]) -> None:
    try:
    ...
    analysis = await analyze_frame_with_ai(session_id, frame_message)
...
async def handle_ping_message(websocket: WebSocket, session_id: str, payload: dict[str, object]) -> None:
    await ping_ai_bridge(session_id)
...
async def live_session_socket(websocket: WebSocket, session_id: str) -> None:
    ...
    try:
        ...
        await send_session_ready(websocket, session_id)            
        while True:
            ...
            if message_type == "frame":
                await handle_live_frame_message(websocket, session_id, payload)
                continue
